      ...
      <Card key={user.id} index={index}>
        <div className="flex flex-col justify-between h-full">
            <div>
              ...
              <p className="text-sm text-slate-400">{user.position || user.role}</p>
            </div>
            {isEmployeeView && (
                ...
                {items.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
                        {isEmployeeView || isAdminView
                            ? (items as User[]).map((user, index) => renderUserCard(user, index)) 
                            : (items as Client[]).map((client, index) => renderClientCard(client, index))}
                    </div>
                ) : (
                    <div className="text-center py-20 text-slate-400">
                        <div className="flex justify-center mb-4">
                            {isEmployeeView || isAdminView ? <UsersIcon className="w-12 h-12 text-slate-600"/> : <BriefcaseIcon className="w-12 h-12 text-slate-600"/> }
                        </div>
                        <h3 className="font-display text-xl text-white">Aucun {isEmployeeView ? 'employé' : isAdminView ? 'administrateur' : 'client'} trouvé</h3>
