...
const NavLink: React.FC<{ icon: React.ReactNode; label: string; isActive: boolean; onClick: () => void; }> = ({ icon, label, isActive, onClick }) => (
  <button
    onClick={onClick}
    className={`w-full flex items-center px-4 py-3 text-sm font-medium transition-all duration-300 rounded-lg group ${
      isActive
...
const Sidebar: React.FC<SidebarProps> = ({ currentUser, activeView, setActiveView, isOpenOnMobile, onCloseMobile, onLogout, onOpenProfileSettings }) => {
  const commonLinks = [
    { view: 'dashboard', label: 'Tableau de bord', icon: <DashboardIcon className="w-6 h-6" /> },
  ];
  
  const messageLink = { view: 'messages', label: 'Messages', icon: <EnvelopeIcon className="w-6 h-6" /> };

  const adminLinks = [
    ...commonLinks,
    messageLink,
    { view: 'employees', label: 'Employés', icon: <UsersIcon className="w-6 h-6" /> },
    { view: 'clients', label: 'Clients', icon: <ClientsIcon className="w-6 h-6" /> },
    { view: 'projects', label: 'Projets', icon: <ProjectsIcon className="w-6 h-6" /> },
  ];
  ...
  const employeeLinks = [
    ...
    messageLink,
    { view: 'projects', label: 'Projets', icon: <ProjectsIcon className="w-6 h-6" /> },
    { view: 'my-clients', label: 'Mes Clients', icon: <ClientsIcon className="w-6 h-6" /> },
  ];

  const clientLinks = [
    ...commonLinks,
    { view: 'my-projects', label: 'Mes Projets', icon: <ProjectsIcon className="w-6 h-6" /> },
    { view: 'my-team', label: 'Mon Équipe', icon: <UsersIcon className="w-6 h-6" /> },
  ];
