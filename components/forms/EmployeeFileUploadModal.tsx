...
interface EmployeeFileUploadModalProps {
  isOpen: boolean;
  onClose: () => void;
  onUpload: (fileData: Omit<ProjectFile, 'id' | 'uploadedBy' | 'lastModified'>) => void;
  projects: Project[];
}
...
const EmployeeFileUploadModal: React.FC<EmployeeFileUploadModalProps> = ({ isOpen, onClose, onUpload, projects }) => {
  ...
  useEffect(() => {
    if (isOpen && projects.length > 0 && !selectedProjectId) {
      // Intentionally not setting a default to force user selection
    }
    ...
    <Modal isOpen={isOpen} onClose={handleClose} title="Téléverser un nouveau fichier">
      <form onSubmit={handleSubmit}>
        <div className="space-y-4">
            <div>
              <label htmlFor="project" className="block text-sm font-medium text-slate-300 mb-1">Sélectionner un projet</label>
              <select
                  id="project"
            ...
            <div className="flex items-center">
              ...
              />
              <label htmlFor="isDeliverableEmployee" className="ml-2 text-sm text-slate-300">Marquer comme livrable pour le client</label>
            </div>
        </div>
...
export default EmployeeFileUploadModal;